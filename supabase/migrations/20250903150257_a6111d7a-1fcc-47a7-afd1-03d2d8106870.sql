...
CREATE INDEX IF NOT EXISTS idx_scans_updated_at ON public.scans(updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_scans_target_url ON public.scans(target_url) WHERE target_url IS NOT NULL;

-- Composite indexes for common query patterns
CREATE INDEX IF NOT EXISTS idx_scans_user_status_created ON public.scans(user_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_scans_user_type_created ON public.scans(user_id, scan_type, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_profiles_encrypted_api_keys_gin ON public.profiles USING GIN(encrypted_api_keys);
CREATE INDEX IF NOT EXISTS idx_profiles_api_keys_gin ON public.profiles USING GIN(api_keys);
CREATE INDEX IF NOT EXISTS idx_scans_results_gin ON public.scans USING GIN(results);
